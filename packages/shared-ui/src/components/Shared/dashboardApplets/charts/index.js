...
const eqArray2D = (a, b) => {
  ...
  if (a.length !== b.length) return false
  for (let i = 0; i < a.length; i++) {
    if (!eqArray(a[i], b[i])) return false
  }
  return true
}
...
export const MixedBarChart = memo(
  ({ id, labels, data1, data2, label1, label2, ratio = 3, rotation, hasLegend }) => {
  ...
  (prev, next) =>
    prev.id === next.id &&
    eqArray(prev.labels, next.labels) &&
    eqArray(prev.data1, next.data1) &&
    eqArray(prev.data2, next.data2) &&
    prev.label1 === next.label1 &&
    prev.label2 === next.label2 &&
    prev.ratio === next.ratio &&
    prev.rotation === next.rotation &&
    prev.hasLegend === next.hasLegend
)

export const HorizontalBarChartDark = memo(
  ...
  (prev, next) =>
    prev.id === next.id &&
    eqArray(prev.labels, next.labels) &&
    eqArray(prev.data, next.data) &&
    prev.label === next.label &&
    prev.color === next.color &&
    prev.hoverColor === next.hoverColor
)

export const CompositeBarChartDark = memo(
  ({ id, labels, data, label, color, hoverColor, ratio = 3 }) => {
    const canvasRef = useRef(null)
    const chartRef = useRef(null)
...
export const MixedColorsBarChartDark = memo(
  ({ id, labels, data, label, ratio = 3 }) => {
    const canvasRef = useRef(null)
    const chartRef = useRef(null)
...
export const CompositeBarChart = memo(
  ({ labels, data, label }) => {
    const canvasRef = useRef(null)
    const chartRef = useRef(null)
  ...
  (prev, next) => eqArray(prev.labels, next.labels) && eqArray(prev.data, next.data) && prev.label === next.label
)
...
export const LineChart = memo(
  ...
  (prev, next) =>
    prev.id === next.id && eqArray(prev.labels, next.labels) && eqArray(prev.data, next.data) && prev.label === next.label
)

export const LineChartDark = memo(
  ...
  (prev, next) =>
    eqArray(prev.labels, next.labels) &&
    eqArray2D(prev.datasets, next.datasets) &&
    eqArray(prev.datasetLabels, next.datasetLabels)
)
...
export const PolarAreaChart = memo(
  ({ id, labels, data, label }) => {
    const ref = useRef(null)
    const inst = useRef(null)
...
export const CompBarChart = memo(
  ({ id, labels, datasets, collapsed }) => {
    const ref = useRef(null)
    const inst = useRef(null)
  ...
  (prev, next) =>
    ...
    eqArray(prev.labels, next.labels) &&
    eqArray(prev.datasets, next.datasets)
)
