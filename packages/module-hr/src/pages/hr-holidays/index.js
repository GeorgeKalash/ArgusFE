...
import { formatDayId, formatDateToYYYYMMDD } from '@argus/shared-domain/src/lib/date-helper'
...
export default function Holidays () {
  ...
  const loadHolidays = async () => {
    ...
    const CDList = items?.list?.length ? 
      items?.list?.map((item, index) => ({...item, id: index + 1, dayId: formatDayId(item.dayId)})) 
