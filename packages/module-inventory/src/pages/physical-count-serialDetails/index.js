      ...
      <VertLayout>
        ...
        <Grow key={formik?.values?.controllerId}>
          <DataGrid
            ...
            value={formik.values.controllerId ? formik.values.rows : initialValues.rows}
            error={formik.errors?.rows}
            initialValues={initialValues?.rows?.[0]}
            columns={columns}
            enableFilters
            disabled={!formik.values.controllerId || formik.values?.SCStatus == 3 || formik.values?.EndofSiteStatus == 3 || formik.values?.status == 3}
            allowDelete={formik.values.controllerId 
              ? formik.values?.SCStatus != 3 && formik.values?.SCWIP != 2 && formik.values?.status != 3 
              : false}
            allowAddNewLine={
