...
const CreditLimitHold = () => {
  ...
  async function fetchGridData(options = {}) {
    ...
    const response = await getRequest({
      extension: FinancialRepository.CreditLimitHold.page,
  ...
  async function fetchWithFilter({ filters, pagination }) {
    if (filters?.qry) {
      return await getRequest({
        extension: FinancialRepository.CreditLimitHold.snapshot,
  ...
  const del = async obj => {
    await postRequest({
      extension: FinancialRepository.CreditLimitHold.del,
  ...
  return (
    <VertLayout>
      <Fixed>
        <RPBGridToolbar onAdd={add} maxAccess={access} reportName={'FICLHO'} filterBy={filterBy} />
      </Fixed>
      <Grow>
        <Table
          name='table'
...
export default CreditLimitHold