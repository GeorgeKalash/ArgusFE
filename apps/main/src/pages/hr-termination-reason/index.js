...
import TerminationForm from './forms/HrTerminationForm'
...
const TermProp = () => {
  ...
  async function fetchGridData(options = {}) {
    ...
    const response = await getRequest({
      extension: EmployeeRepository.TerminationReasons.page,
  ...
  const {
    ...
    datasetId: ResourceIds.TerminationReasons
  })
  ...
  function openForm(recordId) {
    stack({
      ...
      width: 500,
      height: 250,
      title: labels.terminationReason
    })
  }

  const del = async obj => {
    await postRequest({
      extension: EmployeeRepository.TerminationReasons.del,
