...
export default function ModellingForm({ labels, access, setStore, store }) {
  ...
  const { documentType, maxAccess, changeDT } = useDocumentType({
    functionId: SystemFunction.Modelling,
    access: access,
    enabled: !recordId
  })

  const invalidate = useInvalidate({
    endpointId: ProductModelingRepository.Modelling.page
  })
  ...
  const { formik } = useForm({
    ...
    onSubmit: async obj => {
      const res = await postRequest({
        extension: ProductModelingRepository.Modelling.set,
  ...
  async function refetchForm(recordId) {
    const res = await getRequest({
      extension: ProductModelingRepository.Modelling.get,
  ...
  async function onClose() {
    const res = await postRequest({
      extension: ProductModelingRepository.Modelling.close,
      record: JSON.stringify({
        ...formik.values,
  ...
  async function onReopen() {
    const res = await postRequest({
      extension: ProductModelingRepository.Modelling.reopen,
      record: JSON.stringify({
        ...formik.values,
        date: formatDateToApi(formik?.values?.date),
        startDate: formatDateToApi(formik?.values?.startDate),
  ...
  async function onPost() {
    const res = await postRequest({
      extension: ProductModelingRepository.Modelling.post,
      record: JSON.stringify({
        ...formik.values,
        date: formatDateToApi(formik?.values?.date),
        startDate: formatDateToApi(formik?.values?.startDate),
  ...
  async function onUnpost() {
    const res = await postRequest({
      extension: ProductModelingRepository.Modelling.unpost,
      record: JSON.stringify({
        ...formik.values,
        date: formatDateToApi(formik?.values?.date),
        startDate: formatDateToApi(formik?.values?.startDate),
          ...
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <ResourceComboBox
                ...
                parameters={`_startAt=0&_pageSize=1000&_dgId=${SystemFunction.Modelling}`}
                name='dtId'
