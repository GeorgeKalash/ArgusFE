...
export default function RetailTransactionsForm({ labels, posUser, access, recordId, functionId, window }) {
    ...
    validationSchema: yup.object({
      ...
      cash: yup.array().of(
        yup.object({
          ...
          //   .string()
          //   .required()
          //   .test('Credit-Card', 'Credit Card is required', function (value) {
          //     return this.parent.type !== 1 || Boolean(value)
          //   })
        })
      )
    }),
  ...
  async function barcodeSkuSelection(update, row, setItemInfo) {
    ...
    const result = {
      ...
      taxDetails: null //not currect to pass null, it needs to be fixed according to FilteredListByTaxId
    }
    update(result)
    await getItemPriceRow(update, result, DIRTYFIELD_UNIT_PRICE)
  }
  ...
  useEffect(() => {
    ...
    formik.setFieldValue('header.deliveryNotes', address?.deliveryNotes)
  }, [address])
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={10}>
                  <CustomTextField
                    name='taxName'
                    label={labels.tax}
                    value={formik?.values?.header?.taxName}
                    maxAccess={maxAccess}
                    readOnly
                    onChange={formik.handleChange}
                    onClear={() => formik.setFieldValue('header.taxName', '')}
                    error={formik.errors?.header?.axName && Boolean(formik.errors?.header?.taxName)}
                  />
                </Grid>
                <Grid item xs={2}>
                  <FormControlLabel
