...
export default function PurchaseTransactionForm({ labels, access, recordId, functionId, window }) {
  ...
  const columns = [
    {
      ...
      async onChange({ row: { update, newRow } }) {
        ...
        const vendorPrice = await getVendorPrice(newRow)
  ...
  async function fillForm(saTrxPack) {
    ...
    const saTrxTaxes = saTrxPack?.taxCodes
  ...
  async function fillVendorData(object) {
    formik.setValues({
      ...
      header: {
        ...
        tdPct: object?.tradeDiscount ?? DIRTYFIELD_TDPCT,
        taxId: object?.taxId,
  ...
  async function getVendorPrice(object) {
    const res = await getRequest({
      extension: PurchaseRepository.VendorPrice.get,
      parameters: `_itemId=${object.itemId}&_vendorId=${object.vendorId}&_currencyId=${object.currencyId}&_functionId=${functionId}`
    })
  ...
  async function fillItemObject(update, itemPhysProp, itemInfo, vendorPrice) {
    ...
    /**
 * if (pri_resp.result != null && pri_resp.result.currencyId.ToString().Equals(currencyIdParam))
                {

                    baseLaborPrice = (double)pri_resp.result.baseLaborPrice;
                    unitPrice = pri_resp.result.priceList;
                }
                else
                {
