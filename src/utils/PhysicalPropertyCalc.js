// PhysicalPropertyCalculatorCtrl.js

// Constants for identifying which field is dirty
...
// A simple container class to hold physical properties
...
// Controller class that does all the calculations
export class PhysicalPropertyCalculatorCtrl {
  ...
  get(keys) {
    // Create a new PhysicalPropertyCalculator object and fill it from 'keys'
    ...
    this.getRec(rec) // Perform the main calculation
  ...
  getRec(rec) {
    switch (rec.dirtyField) {
      ...
      case DirtyField.DEPTH:
        // If a dimension changed, recalculate volume first
        if (rec.length > 0) {
      ...
      case DirtyField.WEIGHT:
      case DirtyField.DENSITY:
        // If volume, weight, or density is changed directly, recalc
        this.recalc(rec)
        break
    }
  }
